...
class angle_checker {
public:
    explicit angle_checker(const unsigned int histogram_length = 30, const unsigned int num_bins_thr = 3);
...
std::vector<T> angle_checker<T>::get_valid_matches() const {
    ...
    for(unsigned int bin = 0; bin < histogram_length_; ++bin) {
        const bool is_valid_match = std::any_of(bins.begin(), bins.begin() + num_bins_thr_, [bin](const unsigned int i) {
            return bin == i;
        });
        if(is_valid_match) {
