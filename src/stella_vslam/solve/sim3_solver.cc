...
namespace solve {
sim3_solver::sim3_solver(const std::shared_ptr<data::keyframe>& keyfrm_1,
    ...
    const Mat33_t rot_1w = keyfrm_1_->get_rot_cw();
    const Vec3_t trans_1w = keyfrm_1_->get_trans_cw();
    const Mat33_t rot_2w = keyfrm_2_->get_rot_cw();
    const Vec3_t trans_2w = keyfrm_2_->get_trans_cw();
...
void sim3_solver::find_via_ransac(const unsigned int max_num_iter) {
    ...
    for(unsigned int iter = 0; iter < max_num_iter; ++iter) {
        ...
        const auto random_indices = util::create_random_array(3, 0, static_cast<int>(num_common_pts_, -1), random_engine);
        ...
        compute_Sim3(pts_1, pts_2, rot_12_in_sac, trans_12_in_sac, scale_12_in_sac, rot_21_in_sac, trans_21_in_sac, scale_21_in_sac);

        std::vector<bool> inliers;
        const auto num_inliers = count_inliers(rot_12_in_sac, trans_12_in_sac, scale_12_in_sac, rot_21_in_sac, trans_21_in_sac, scale_21_in_sac, inliers);
...
void sim3_solver::compute_Sim3(const Mat33_t& pts_1,
                                const Mat33_t& pts_2,
                                Mat33_t& rot_12,
                                Vec3_t& trans_12,
                                float& scale_12,
    ...
    Eigen::Matrix4d N;
    N << (Sxx + Syy + Szz), (Syz - Szy), (Szx - Sxz), (Sxy - Syx), (Syz - Szy), (Sxx - Syy - Szz), (Sxy + Syx), (Szx + Sxz), (Szx - Sxz), (Sxy + Syx), (-Sxx + Syy - Szz), (Syz + Szy), (Sxy - Syx), (Szx + Sxz), (Syz + Szy), (-Sxx - Syy + Szz);
...
unsigned int sim3_solver::count_inliers(const Mat33_t& rot_12,
