...
__device__ __forceinline__ bool isKeyPoint(int mask1, int mask2) {
    return (__popc(mask1) > 8 && (c_table[(mask1 >> 3) - 64] & (1 << (mask1 & 7)))) || (__popc(mask2) > 8 && (c_table[(mask2 >> 3) - 63] & (1 << (mask2 & 7))));
}
...
__global__ void tileCalcKeypoints_kernel(const PtrStepSzb img,
                                        short2* kpLoc,
                                        float* kpScore,
...
IC_Angle::IC_Angle(unsigned int maxKeypoints) 
    : maxKeypoints(maxKeypoints) {
    checkCudaErrors(cudaStreamCreate(&stream));
