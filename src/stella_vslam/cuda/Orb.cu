...
#include <stella_vslam/util/cuda_util.h>
#include <stdio.h>
...
#define GET_VALUE(idx) image(loc.y + __float2int_rn(pattern[idx].x * b + pattern[idx].y * a), loc.x + __float2int_rn(pattern[idx].x * a - pattern[idx].y * b));
...
GpuOrb::GpuOrb(int maxKeypoints) 
    : maxKeypoints(maxKeypoints)
    , descriptors(maxKeypoints, 32, CV_8UC1) {
